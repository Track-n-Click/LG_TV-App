import { fetchPodcastById } from "./podcastService.js";
  ...
  if (id) {
    ...
    fetchPodcastById(id)
      .then((data) => {
        ...
        albumTitleElement.textContent = data.podcast.title;
        albumDescriptionElement.textContent = data.podcast.description;
        ...
        data.episodes.data.forEach((episode) => {
          const listItem = document.createElement("li");
          listItem.innerHTML = `
          <div class="track-link" data-src="${episode.stream_url}" data-title="${episode.title}" data-artist="${episode.artist.name}" data-artwork="${episode.podcast.artwork_url}">
            <img src="${episode.podcast.artwork_url}" alt="${episode.title} Artwork" class="track-artwork" style="width: 50px; height: 50px; margin-right: 10px;" />
        ...
        const firstEpisode = data.episodes.data[0];
        if (firstEpisode) {
          playTrack(
            firstEpisode.stream_url,
            firstEpisode.title,
            firstEpisode.artist.name,
            firstEpisode.podcast.artwork_url
          );
        }
